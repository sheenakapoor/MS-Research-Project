...
# find /jet/home/sheenak/tmp_ondemand_ocean_atm200005p_symlink/mkhan2/SEVERI_2016/ORD46294/ \( -name "*120000305SVMSGE1MD.nc" \) -exec cp {} nc_noon \;  
...
def regrid(file, file_out):
    ...
    area_def = geometry.AreaDefinition.from_extent(area_id='azores',projection='+proj=eqc',shape=(3636,3636),area_extent=(-65,25,0,60),units='deg')
    ...
    plt.pcolormesh(lon[0,:],lat[:,0],result_data_quick, cmap='RdBu_r', vmin=0)
    ax.set_xlabel("")
    ax.set_ylabel("")
    ...
    fig.savefig("/jet/home/sheenak/tmp_ondemand_ocean_atm200005p_symlink/sheenak/nc_noon/noon_regridded/png_regridded/"+str(file.replace(".nc", ".png")))
    plt.show()
    plt.close("all")
    ...
    my_cube = iris.cube.Cube(result_data_quick, "atmosphere_optical_thickness_due_to_cloud",  dim_coords_and_dims=[(latitude,0), (longitude,1)])
...
for index, file in enumerate(filename):
    ...
    regrid(file, "/jet/home/sheenak/tmp_ondemand_ocean_atm200005p_symlink/sheenak/nc_noon/noon_regridded/"+str(file.replace(".nc", "_new.nc")))
